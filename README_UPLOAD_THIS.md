...
- Added only DURBIN Client 1.21.11:
  https://github.com/iamcocotheking-cmd/luncher/releases/download/cosa/1.21.11.zip
- DURBIN Client profile uses LTW renderer by default.
- Launcher default renderer is LTW (`opengles3_ltw`).


## v39 Ads + rank shop/profile picture
- Fixed home ad image loading from Firebase.
- Removed Latest News from main menu.
- Added rank profile pictures.
- Added rank selling fields in dashboard and app:
  price, sellLink, forSale.


## v40 Clean rank PFP + polished download page + fade + tasks
- Removed rank selling system.
- Kept rank profile pictures.
- Removed sell fields from backend dashboard.
- Redesigned DURBIN Client download area.
- Download progress now shows in Tasks.
- Added smoother fade animation when switching launcher areas.


## v41 DURBIN Client name fix
- Changed COSA labels back to DURBIN Client.
- Created profile now says DURBIN Client 1.21.11.
- Download page now says DURBIN Client.
- Same 1.21.11 ZIP link is kept.


## v42 CSS loader animation
- Replaced the loading animation with a Compose version of your uploaded 3-arc CSS loader.
- Uses the same red tilted spinning arc idea.


## v43 runOnUiThread delayed compile fix
- Fixed `Too many arguments for runOnUiThread`.
- Uses `Handler(Looper.getMainLooper()).postDelayed(...)` for delayed task cleanup.
- Keeps v42 CSS loader animation.
